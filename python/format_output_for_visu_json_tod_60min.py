...
def main(args):
    ...
    # Define 60-minute intervals in a day
    intervals = pd.date_range("00:00", "23:45", freq="60T").time
    interval_index_map = {interval: idx for idx, interval in enumerate(intervals)}
    ...
    for sensor, sensor_data in data.items():
        ...
        # Apply time-of-day filtering if specified
        if args.daytime_period is not None:
        ...
        # Assign each entry to a 60-minute interval
        df["interval"] = df["time"].apply(
            lambda x: min(intervals, key=lambda t: abs(datetime.combine(datetime.min, t) - datetime.combine(datetime.min, x)))
        )
        ...
        # Group by interval and compute averages
        grouped = df.groupby("interval")[["leq", "laeq", "t", "v", "b"]].mean().reset_index()
        grouped["interval"] = grouped["interval"].map(interval_index_map)  # Convert time to index
        grouped = grouped.sort_values("interval").reset_index(drop=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process soundscape data into time-of-day intervals.")
